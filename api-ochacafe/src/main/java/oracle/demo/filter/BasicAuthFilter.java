...
public class BasicAuthFilter implements ContainerRequestFilter {
    ...
    /**
     * In order to let this sample work on WebLogig Server, you must set the
     * enforce-valid-basic-auth-credentials Flag to false Please refer to
    ...
    public void filter(ContainerRequestContext context) throws IOException {
        ...
        if (2 != headerElements.length || !headerElements[0].equalsIgnoreCase(AUTH_BASIC)) {
            throw new NotAuthorizedException(AUTH_ERROR);
        }
        ...
        if (2 != userpw.length || null == userpw[0] || null == userpw[1] || !userpw[0].equals(AUTH_USERNAME)
                || !userpw[1].equals(AUTH_PASSWORD)) {
            throw new NotAuthorizedException(AUTH_ERROR);
        }
    }
